...
BERT_DotBatch = Tuple[torch.LongTensor, torch.LongTensor, torch.LongTensor]


class BERTDotDataProcessor(DataProcessor):
...
class BERTDotRanker(Ranker):
    ...
    def __init__(self, lr: float, warmup_steps: int, hparams: Dict[str, Any],) -> None:
        ...
        self.bert: BertModel = BertModel.from_pretrained(hparams["bert_model"], return_dict=True)
